...
static void dsp_dma_run(DSPDMAState *s)
{
    ...
    while (!(s->next_block & NODE_POINTER_EOL)) {
        ...
        if (addr < 0x1800) {
            ...
            assert(!"DSP DMA block address outside X/Y/P windows");
        }
        ...
        default:
            ...
            assert(!"Unknown DSP DMA transfer format");
            break;
        }
        ...
        if (dsp_offset < 0x1800) {
            ...
            assert(!"DSP DMA access outside valid address range");
        }
        ...
        if (direction) {
                    ...
                    for (int ch = 0; ch < channel_count; ch++) {
                        ...
                        case 4:
                            *(uint32_t*)(scratch_buf + i*4*channel_count + ch*4) = v;
                            break;
                        default:
                            assert(!"Unsupported DSP DMA item size (multi-channel)");
                            break;
                        }
                    }
                }
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    case 4:
                        *(uint32_t*)(scratch_buf + i*4) = v;
                        break;
                    default:
                        assert(!"Unsupported DSP DMA item size");
                        break;
                    }
                }
            ...
            default:
                ...
                assert(!"Unknown DSP DMA source buffer id");
                break;
            }
        } else {
            ...
            if (buf_id == 0xe) {
                ...
                assert(!"Unhandled DSP DMA destination buffer id");
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                default:
                    v = 0;
                    assert(!"Unsupported DSP DMA item size on read");
                    break;
                }
...
uint32_t dsp_dma_read(DSPDMAState *s, DSPDMARegister reg)
{
    ...
    case DMA_NEXT_BLOCK:
        return s->next_block;
    default:
        assert(!"Read of unknown DSP DMA register");
    }
    return 0;
}
...
void dsp_dma_write(DSPDMAState *s, DSPDMARegister reg, uint32_t v)
{
    ...
    case DMA_CONTROL:
        ...
        case DMA_CONTROL_ACTION_UNFREEZE:
            ...
            break;
        default:
            assert(!"Unknown DSP DMA control action");
            break;
        }
        dsp_dma_run(s);
    ...
    case DMA_NEXT_BLOCK:
        ...
        break;
    default:
        assert(!"Write to unknown DSP DMA register");
    }
}
