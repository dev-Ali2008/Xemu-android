...
static void voice_lock(MCPXAPUState *d, uint16_t v, bool lock)
{
    ...
    /*
     * Only a release can unblock anyone: the sole waiter,
     * voice_work_acquire_voice_lock_for_processing(), loops while
     * is_voice_locked(v) is true. Broadcasting when a voice is *acquired* woke
     * every waiter just to re-check, find the voice still locked and sleep
     * again - a thundering herd that showed up as ~11.6% of all CPU spinning
     * in this function during a Forza race.
     */
    if (!lock) {
...
static void fe_method(MCPXAPUState *d, uint32_t method, uint32_t argument)
{
    ...
    case SE2FE_IDLE_VOICE:
        ...
        break;
    default:
        assert(!"Unhandled voice processor method");
        break;
    }
}
...
/*
 * Voice sample fetch reads guest RAM one container at a time - per channel, per
 * sample, per voice, per frame.  Routing each of those through
 * ld*_phys(&address_space_memory, ...) pays a full flatview translation, an RCU
 * read section and a dynamic TLS resolve to fetch two bytes of PCM.  Measured
 * on Forza: address_space_ldl_internal 5.7% and address_space_translate_internal
 * 1.5% of all process CPU, plus get_ptr_rcu_reader at 13.4% - and 63% of the
 * linker's tlsdesc_resolver_dynamic time is reached through that RCU reader,
 * which is TLS cost that cannot be attacked directly because
 * -ftls-model=initial-exec stops bionic from dlopen'ing libxemu at all.
 *
 * The APU already keeps a direct host pointer to the RAM region (d->ram_ptr,
 * which the ADPCM streaming path memcpy's straight out of), so resolve against
 * it and read directly.  Returns NULL when the access is not wholly inside RAM
 * so the caller can fall back to the address space, keeping MMIO and
 * out-of-range reads behaving exactly as before.
 */
static inline const uint8_t *voice_ram_host_ptr(MCPXAPUState *d, hwaddr addr,
                                                unsigned int size)
{
    if (!d->ram_ptr) {
        return NULL;
                           ...
                           unsigned int max_sge, uint32_t addr)
{
    ...
    hwaddr prd_addr = sge_base + entry * 4 * 2;
    const uint8_t *prd_host = voice_ram_host_ptr(d, prd_addr, 4);
    uint32_t prd_address = prd_host ?
        ldl_le_p(prd_host) : ldl_le_phys(&address_space_memory, prd_addr);
                           ...
                           uint32_t count_mask, uint32_t cur_mask)
{
    ...
    default:
        ...
        assert(!"Unknown voice envelope state");
        return 0.0f;
    }
}
                       ...
                       int num_samples_requested)
{
    ...
    for (; (sample_count < num_samples_requested) && (cbo <= ebo);
        ...
        if (adpcm) {
            ...
            if (adpcm_block_index != block_index) {
                ...
                if (stream) {
                    ...
                    for (unsigned int word_index = 0;
                        ...
                        const uint8_t *hp = voice_ram_host_ptr(d, addr, 4);
                        adpcm_block[word_index] = hp ?
                            ldl_le_p(hp) :
            ...
            /* Bytes touched per read, which is not always container_size: a
             * voice may pair a small container with a wider sample size. */
            static const unsigned int sample_read_sizes[4] = { 1, 2, 4, 4 };
            unsigned int read_size = sample_read_sizes[sample_size];

            for (unsigned int channel = 0; channel < channels; channel++) {
                uint32_t ival;
                float fval;
                const uint8_t *hp = voice_ram_host_ptr(d, addr, read_size);
                switch (sample_size) {
                ...
                case NV_PAVS_VOICE_CFG_FMT_SAMPLE_SIZE_S16:
                    ival = hp ? lduw_le_p(hp)
                              : lduw_le_phys(&address_space_memory, addr);
                ...
                case NV_PAVS_VOICE_CFG_FMT_SAMPLE_SIZE_S32:
                    ...
                    break;
                default:
                    assert(!"Unsupported voice sample size");
                    break;
                }
