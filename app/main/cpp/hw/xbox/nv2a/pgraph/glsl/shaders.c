...
ShaderState pgraph_glsl_get_shader_state(PGRAPHState *pg)
{
    ...
    if (state.vsh.is_fixed_function && state.vsh.fixed_function.skinning != SKINNING_OFF) {
        /* Determine bone matrix convention by examining actual constant
         * content at compile time, rather than relying on the global
         * bone_matrix_convention flag which can be toggled by unrelated
         * inverse-matrix uploads between draw calls (e.g. environment
         * maps, lighting matrices). We compare the number of MMAT vs
         * IMMAT slots that contain plausible bone data (non-identity). */
        static const int mmat_bases[4] = {
            NV_IGRAPH_XF_XFCTX_MMAT0, NV_IGRAPH_XF_XFCTX_MMAT1,
            NV_IGRAPH_XF_XFCTX_MMAT2, NV_IGRAPH_XF_XFCTX_MMAT3,
        };
        static const int immat_bases[4] = {
            NV_IGRAPH_XF_XFCTX_IMMAT0, NV_IGRAPH_XF_XFCTX_IMMAT1,
            NV_IGRAPH_XF_XFCTX_IMMAT2, NV_IGRAPH_XF_XFCTX_IMMAT3,
        };
        int mmat_score = 0, immat_score = 0;
        for (int i = 0; i < 4; i++) {
            float *mm = (float *)&pg->vsh_constants[mmat_bases[i]][0];
            float *im = (float *)&pg->vsh_constants[immat_bases[i]][0];
            float mm_sum = 0.0f, im_sum = 0.0f;
            for (int k = 0; k < 16; k++) {
                mm_sum += mm[k] * mm[k];
                im_sum += im[k] * im[k];
            }
            float mm_diag = mm[0] * mm[0] + mm[5] * mm[5]
                          + mm[10] * mm[10] + mm[15] * mm[15];
            float im_diag = im[0] * im[0] + im[5] * im[5]
                          + im[10] * im[10] + im[15] * im[15];
            /* Identity diagonal squares sum to 4.0. Non-identity sum
             * indicates a real transform matrix. Also check total sum
             * to catch matrices with significant off-diagonal data. */
            if (mm_sum > 0.001f && (mm_diag < 3.99f || mm_diag > 4.01f
                || mm_sum - mm_diag > 0.001f)) {
                mmat_score++;
            }
            if (im_sum > 0.001f && (im_diag < 3.99f || im_diag > 4.01f
                || im_sum - im_diag > 0.001f)) {
                immat_score++;
            }
        }
        if (immat_score > mmat_score) {
            state.vsh.fixed_function.bone_matrix_convention = 1;
        } else {
            state.vsh.fixed_function.bone_matrix_convention = 0;
        }
    }
