...
#ifdef __ANDROID__
    /*
     * Final gate for stock Qualcomm drivers.
     *
     * Both of the two-write descriptor branches fault inside the driver's
     * vkUpdateDescriptorSets() (SIGSEGV at null+0x28 on Adreno 750, reached
     * from pgraph_vk_update_descriptor_sets -> begin_pre_draw). The feature
     * flags are already supposed to be off for this hardware - set in
     * instance.c alongside the existing push-descriptor and EDS3 guards - but
     * a renderer rebuilt mid-session was still reaching those branches, so the
     * decision point itself is gated rather than trusting the flags.
     *
     * Only stock Adreno is affected; an updated driver keeps the fast paths.
     * Falling through means the plain descriptor-set path, which is the
     * upstream default and is what the OpenGL-equivalent workload already
     * runs through without incident.
     */
    {
...
{
    /*
     * Deliberately does nothing. Kept as a call site so the frame-reset path
     * reads the same as before.
     *
     * This used to vkDestroyDescriptorPool() every overflow pool and then
     * g_realloc() the descriptor arrays back down, on the assumption stated at
     * the call site that "all GPU work is complete". Destroying a pool
     * implicitly frees every descriptor set allocated from it, so any set the
     * driver still referenced became a dangling - but non-NULL - handle, and
     * the next vkUpdateDescriptorSets() faulted inside the driver (SIGSEGV at
     * null+0x28 on Adreno 750, reproducible in Forza Motorsport).
     *
     * That assumption does not hold in this renderer: unlike upstream xemu it
     * submits through render_thread.c / submit_worker.c, so frames can still
     * be in flight here. Upstream never destroys a descriptor pool mid-session
     * at all - it allocates its sets once and frees them only when the
     * renderer is torn down - which is why upstream does not hit this.
     *
     * Restoring that invariant: pools now live for the renderer's lifetime and
     * are destroyed in pgraph_vk_finalize_shaders() as before. The cost is
     * that peak descriptor memory stays allocated instead of being handed
     * back, which is the same trade upstream already makes.
     */
    (void)r;
}

/*
 * Crash diagnostic (XaniteOG). A null dstSet or buffer handle reaching
 * vkUpdateDescriptorSets() is dereferenced inside the driver, which takes the
 * whole process down with SIGSEGV before any validation can report it - on an
 * Adreno 750 this shows up as a fault at address 0x28 with no other clue.
 *
 * Purely observational: this only logs, and only when a handle is already
 * bad, so a healthy frame costs two comparisons per write and prints nothing.
 * That matters here because this runs per draw call.
 */
/*
 * Diagnostic counters. Only ever read and written from this function on the
 * PFIFO thread, and only used for logging, so plain variables are fine - no
 * synchronisation is added to a hot path for the sake of a counter.
 */
static unsigned long g_diag_update_seq;      /* descriptor updates so far */
static unsigned long g_diag_wrap_seq;        /* ring wraps so far */
static unsigned long g_diag_since_wrap = ~0UL; /* updates since the last wrap */

/* Number of updates after a wrap that get logged unconditionally. */
#define DIAG_POST_WRAP_LOGGED 3

/*
 * Fires the first time either two-write branch is entered, and again whenever
 * the feature flags differ from what they were when first observed.
 *
 * These flags are single-byte bools inside PGRAPHVkState. If they read as set
 * here while DIAG-PATH reported them clear for the same renderer, nothing in
 * the descriptor code did that - something wrote over the struct. That is a
 * very different bug from the descriptor lifetime issues chased so far, and
 * this is the cheapest way to tell the two apart.
 */
/**
 * Makes room in a descriptor ring that has run out.
 *
 * Recycling from index 0 is only legal once nothing can still reference those
 * sets. Waiting on fences proves submitted work is done, but says nothing
 * about a command buffer that is open and recording - and updating a set that
 * such a buffer has already bound invalidates it. The Khronos validation layer
 * reported exactly that:
 *
 *   VkDescriptorSet ... was destroyed or updated without UPDATE_AFTER_BIND
 *
 * UPDATE_AFTER_BIND cannot be used to make it legal, because the spec forbids
 * it for VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, which these UBO bindings
 * use. So inside a command buffer the ring is grown instead of recycled -
 * wiring up grow_descriptor_ring(), which was written for this and had no
 * callers, leaving recycling as the only path.
 *
 * Returns true when the caller may proceed with *ds_index_ptr, false if room
 * could not be made and the descriptor update must be skipped this draw.
 */
static bool descriptor_ring_recycle_or_grow(PGRAPHState *pg, PGRAPHVkState *r,
                                            VkDescriptorSetLayout layout,
                                            VkDescriptorSet **sets_ptr,
                                            int *index_ptr, int *count_ptr,
                                            bool include_samplers)
{
    if (r->in_command_buffer) {
        if (grow_descriptor_ring(r, layout, sets_ptr, count_ptr,
                                 include_samplers)) {
            return true;
        }
        /*
         * Out of room and unable to grow. Skipping the update leaves the
         * previous descriptor contents bound, which renders the draw with
         * stale state - visibly wrong at worst. Recycling here would corrupt
         * the command buffer and take the process down.
         */
        VK_LOG_ERROR("DIAG-RING: exhausted inside a command buffer and could "
                     "not grow (index=%d count=%d) - skipping update",
                     *index_ptr, *count_ptr);
        return false;
    }
...
static void diag_report_branch_taken(const char *path, PGRAPHVkState *r)
{
    static const PGRAPHVkState *seen;
    static int seen_bindless = -1;
    static int seen_push = -1;

    int bindless = 0;
#if OPT_BINDLESS_TEXTURES
    bindless = (int)r->bindless_textures_supported;
#endif
    int push = (int)r->push_descriptors_supported;

    if (seen == r && bindless == seen_bindless && push == seen_push) {
        return;
    }
    seen = r;
    seen_bindless = bindless;
    seen_push = push;

    VK_LOG_ERROR("DIAG-BRANCH: entered '%s' branch | bindless_supported=%d "
                 "push_supported=%d | vendorID=0x%x | r=%p",
                 path, bindless, push, r->device_props.vendorID, (void *)r);
}

static void diag_check_ubo_writes(const char *path,
                                  const VkWriteDescriptorSet *writes,
                                  const VkDescriptorBufferInfo *infos,
                                  int count, int ds_index, int ds_count,
                                  VkDeviceSize buffer_size)
{
    for (int i = 0; i < count; i++) {
        /*
         * range must also fit the buffer: a descriptor whose range overruns
         * its allocation is invalid, and the driver dereferences internal
         * state for it before any validation layer would get a say.
         */
        bool ok = writes[i].dstSet != VK_NULL_HANDLE &&
                  infos[i].buffer != VK_NULL_HANDLE && infos[i].range != 0 &&
                  infos[i].offset + infos[i].range <= buffer_size;

        /*
         * Log the first few updates after a ring wrap even when they look
         * fine. If the crash lands right after a wrap, the descriptor set
         * being recycled while still referenced by an in-flight command
         * buffer is the explanation, and only the correlation shows that.
         */
        if (ok && g_diag_since_wrap >= DIAG_POST_WRAP_LOGGED) {
            continue;
        }

        VK_LOG_ERROR("DIAG-UBO[%s] %s write %d/%d: dstSet=%p buffer=%p "
                     "offset=%llu range=%llu buf_size=%llu binding=%u "
                     "ds_index=%d ds_count=%d update=%lu wrap=%lu "
                     "since_wrap=%lu",
                     path, ok ? "postwrap" : "INVALID", i, count,
                     (void *)writes[i].dstSet, (void *)infos[i].buffer,
                     (unsigned long long)infos[i].offset,
                     (unsigned long long)infos[i].range,
                     (unsigned long long)buffer_size,
                     (unsigned)writes[i].dstBinding, ds_index, ds_count,
                     g_diag_update_seq, g_diag_wrap_seq, g_diag_since_wrap);
    }
}
...
void pgraph_vk_update_descriptor_sets(PGRAPHState *pg)
{
    ...
    /*
     * Report the descriptor path actually taken, once per process.
     *
     * The settings say bindless is off and stock Qualcomm drivers have push
     * descriptors force-disabled, which should put us on the plain path - yet
     * the crash lands in one of the two-write branches. Rather than keep
     * inferring runtime state from source, print it.
     */
    {
        /*
         * Keyed on the renderer instance, not a plain once-per-process flag.
         * The renderer is torn down and rebuilt during a session, and a
         * once-only log reported the first instance's configuration while a
         * later one was what actually crashed.
         */
        static const PGRAPHVkState *logged_for;
        static const char *last_branch;
        const char *branch_now;
#if OPT_BINDLESS_TEXTURES
        if (r->bindless_textures_supported) {
            branch_now = "bindless (2 writes)";
        } else
#endif
        if (push_desc) {
            branch_now = "push (2 writes)";
        } else {
            branch_now = "plain (6 writes)";
        }
        /*
         * Keying on the renderer pointer alone was not enough: a rebuilt
         * renderer is frequently allocated at the same address, so the new
         * instance was silently skipped and this reported the configuration
         * of a renderer that never crashed. Also log whenever the chosen
         * branch changes, which is the thing actually worth knowing.
         */
        if (logged_for != r || last_branch != branch_now) {
            logged_for = r;
            last_branch = branch_now;
            const char *branch = branch_now;
            VK_LOG_ERROR(
                "DIAG-PATH: branch=%s | bindless_supported=%d push_supported=%d "
                "push_desc=%d | vendorID=0x%x deviceID=0x%x driverVersion=0x%x "
                "apiVersion=%u.%u.%u device='%s' | OPT_BINDLESS_TEXTURES=%d",
                branch,
#if OPT_BINDLESS_TEXTURES
                (int)r->bindless_textures_supported,
#else
                -1,
#endif
                (int)r->push_descriptors_supported, (int)push_desc,
                r->device_props.vendorID, r->device_props.deviceID,
                ...
                r->device_props.deviceName, OPT_BINDLESS_TEXTURES);
        }
    }
    ...
    if (need_new_descriptor_set && *ds_index_ptr >= *ds_count_ptr) {
        ...
        if (!descriptor_ring_recycle_or_grow(pg, r, ds_layout, ds_array_ptr,
                                             ds_index_ptr, ds_count_ptr,
                                             ds_include_samplers)) {
            return;
        }
    }
    ...
    g_diag_update_seq++;
    if (g_diag_since_wrap != ~0UL) {
        g_diag_since_wrap++;
    }

    if (*ds_index_ptr >= *ds_count_ptr) {
        OPT_STAT_INC(buf_ds_full);
        /* Observational only - the wrap itself is unchanged below. */
        g_diag_wrap_seq++;
        g_diag_since_wrap = 0;
        VK_LOG_ERROR("DIAG-WRAP #%lu at update %lu: recycling descriptor ring "
                     "(ds_index=%d ds_count=%d)",
                     g_diag_wrap_seq, g_diag_update_seq, *ds_index_ptr,
                     *ds_count_ptr);
        if (!descriptor_ring_recycle_or_grow(pg, r, ds_layout, ds_array_ptr,
                                             ds_index_ptr, ds_count_ptr,
                                             ds_include_samplers)) {
    ...
    if (r->bindless_textures_supported) {
        ...
        /*
         * Unambiguous branch marker. DIAG-PATH reports this renderer as being
         * on the plain path with both feature flags clear, yet the crash lands
         * in a two-write branch - which is only reachable when one of those
         * flags is set. If this line ever prints, the flags changed after
         * DIAG-PATH read them, which means something is writing over
         * PGRAPHVkState rather than the descriptor logic being at fault.
         */
        diag_report_branch_taken("bindless", r);
        diag_check_ubo_writes("bindless", descriptor_writes, ubo_buffer_infos,
                              2, *ds_index_ptr, *ds_count_ptr,
                              r->storage_buffers[BUFFER_UNIFORM].buffer_size);
    ...
    if (push_desc) {
        ...
        diag_report_branch_taken("push", r);
        diag_check_ubo_writes("push", descriptor_writes, ubo_buffer_infos, 2,
                              *ds_index_ptr, *ds_count_ptr,
                              r->storage_buffers[BUFFER_UNIFORM].buffer_size);
        vkUpdateDescriptorSets(r->device, 2, descriptor_writes, 0, NULL);
    } else {
        ...
        /*
         * Built up rather than fixed at 6: a texture binding with no usable
         * image view or sampler is omitted instead of being passed to the
         * driver as a null handle.
         */
        ...
        for (int i = 0; i < NV2A_MAX_TEXTURES; i++) {
            /*
             * A COMBINED_IMAGE_SAMPLER descriptor requires both a valid image
             * view and a valid sampler. Handing the driver a null handle here
             * is not diagnosed by any validation we can reach - it faults
             * inside vkUpdateDescriptorSets() and takes the process with it
             * (SIGSEGV at null+0x28 on Adreno 750, reproducible in Forza
             * Motorsport).
             *
             * The direct-surface path is what made this reachable: upstream
             * xemu always sources the view from texture_bindings[i], while
             * this renderer substitutes tex_surface_direct_views[i] when the
             * per-unit flag is set - and that view can legitimately be
             * VK_NULL_HANDLE. The pre-existing check below only ever looked at
             * texture_bindings[i]->image_view, i.e. the branch NOT taken in
             * that case, which is why it never once fired.
             *
             * Falling back to the regular binding keeps the draw correct
             * rather than dropping it; the direct-surface view is an
             * optimisation, not the only source of the texture.
             */
            bool direct = r->tex_surface_direct[i] &&
                          r->tex_surface_direct_views[i] != VK_NULL_HANDLE;

            if (r->tex_surface_direct[i] && !direct) {
                VK_LOG_ERROR("DIAG-TEX: tex[%d] direct-surface view is NULL, "
                             "falling back to texture binding (ds_index=%d)",
                             i, *ds_index_ptr);
            }

            VkImageView view = direct ? r->tex_surface_direct_views[i]
                                      : r->texture_bindings[i]->image_view;
            VkSampler sampler = r->texture_bindings[i]->sampler;

            if (view == VK_NULL_HANDLE || sampler == VK_NULL_HANDLE) {
                VK_LOG_ERROR("DIAG-TEX: tex[%d] unusable descriptor "
                             "(view=%p sampler=%p direct=%d) - skipping "
                             "descriptor update to avoid faulting the driver",
                             i, (void *)view, (void *)sampler,
                             (int)r->tex_surface_direct[i]);
            }
            ...
            if (view == VK_NULL_HANDLE || sampler == VK_NULL_HANDLE) {
                /*
                 * Leave this binding out of the batch entirely. The previous
                 * contents of the set stay bound, which is survivable; passing
                 * the null handle through is not.
                 */
                continue;
            }
        ...
        vkUpdateDescriptorSets(r->device, write_count, descriptor_writes, 0,
                               NULL);
    }
