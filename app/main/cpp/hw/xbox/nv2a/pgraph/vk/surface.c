...
const int num_invalid_surfaces_to_keep = 20;
const int max_surface_frame_time_delta = 8;
...
void pgraph_vk_reload_surface_scale_factor(PGRAPHState *pg)
{
    ...
    float new_factor = MAX(factor, 0.5f);
...
static void destroy_surface_image(PGRAPHVkState *r, SurfaceBinding *surface)
{
    ...
    float sf = g_nv2a->pgraph.surface_scale_factor;
    if (sf > 1.0f) {
        w = (unsigned int)((float)w * sf);
        h = (unsigned int)((float)h * sf);
    } else if (sf < 1.0f) {
        w = (unsigned int)((float)w * sf);
        h = (unsigned int)((float)h * sf);
        if (w < 1) w = 1;
...
static const int max_shelved_surfaces = 40;
