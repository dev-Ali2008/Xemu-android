                                         ...
                                         TextureBinding *texture)
{
    ...
    /* Submit before recording this copy if the transient-view retirement
     * list is full. This must happen before cmd is acquired because finish()
     * rotates command buffers. */
    if (r->deferred_image_view_count[r->current_frame] >=
        MAX_DEFERRED_IMAGE_VIEWS) {
        pgraph_vk_finish(pg, VK_FINISH_REASON_NEED_BUFFER_SPACE);
        pgraph_vk_flush_all_frames(pg);
    }
    ...
    /* Create a depth-only image view for compute sampling. */
    ...
    /* Do not destroy depth_view here. The descriptor and command buffer still
     * reference it until this frame's fence signals. Immediate destruction
     * caused intermittent black geometry/artifacts on Adreno drivers. */
    r->deferred_image_views[r->current_frame]
                           [r->deferred_image_view_count[r->current_frame]++] =
        depth_view;
...
static void create_texture(PGRAPHState *pg, int texture_idx)
{
    ...
    if (!surface_to_texture) {
        ...
        if (texture_palette_data_size && check_texture_dirty(
    ...
    if (binding_found) {
        ...
        if (surface_to_texture) {
            if (surface->draw_time != snode->draw_time) {
                ...
                did_s2t_copy = true;
            } else if (OPT_SURFACE_DIRECT_TEX &&
                       (surface->color ||
...
bool pgraph_vk_check_textures_fast_skip(PGRAPHState *pg)
{
    ...
    for (int i = 0; i < NV2A_MAX_TEXTURES; i++) {
        ...
        if (binding->key.palette_length > 0 && check_texture_dirty(
                d, binding->key.palette_vram_offset,
        ...
        if (vram_dirty) {
            ...
            binding->dirty_check_result = true;
            return false;
        }
    }
    return true;
