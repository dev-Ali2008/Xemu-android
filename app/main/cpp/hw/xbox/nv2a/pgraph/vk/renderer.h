...
/* Fork-only: bind a render surface's image view straight in as a texture
 * instead of copying it out through the normal texture path. Upstream has no
 * equivalent. Set to 0 to force the copy path - used to test whether the
 * striping/hatching artifacts seen on Turnip come from sampling surfaces
 * (especially depth/zeta) directly. */
#define OPT_SURFACE_DIRECT_TEX   1
...
typedef struct PGRAPHVkState {
    ...
    bool push_tex_dirty;
    /* Pipeline layout the currently recorded push descriptors belong to.
     * Push descriptors survive a pipeline bind as long as the layout stays
     * compatible, so only a layout change needs to force a re-push. */
    VkPipelineLayout push_desc_layout;
...
#define FB_CACHE_MAX 32
    ...
    /* Transient depth-only views referenced by recorded command buffers.
     * Vulkan objects must remain alive until the frame fence signals. */
#define MAX_DEFERRED_IMAGE_VIEWS 512
    VkImageView deferred_image_views[NUM_SUBMIT_FRAMES][MAX_DEFERRED_IMAGE_VIEWS];
    int deferred_image_view_count[NUM_SUBMIT_FRAMES];
...
void pgraph_vk_set_surface_scale_factor(NV2AState *d, float scale);
float pgraph_vk_get_surface_scale_factor(NV2AState *d);
