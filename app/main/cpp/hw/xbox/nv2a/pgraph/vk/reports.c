...
void pgraph_vk_process_pending_reports(NV2AState *d)
{
    ...
    /* Upstream xemu ebe34071: an idle pushbuffer is not enough reason to
     * force a GPU finish. Only stall when the guest is actually waiting for
     * a report; otherwise this path serializes every quiet command-buffer
     * period and is particularly expensive on tiled mobile GPUs. */
