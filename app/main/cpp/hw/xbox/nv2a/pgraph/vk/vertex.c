                                        ...
                                        void *data, VkDeviceSize size)
{
    ...
    /*
     * A frame has its own backing buffer, but multiple draws recorded into
     * that frame still share it.  Games commonly rewrite one small vertex
     * range for every font glyph/menu quad.  If that range has already been
     * referenced by the current command buffer, overwriting it here makes all
     * earlier draws consume the newest glyph geometry and collapses menu text
     * into scattered characters.  The renderer-level bitmap is scoped to one
     * command buffer; the per-frame bitmap below separately tracks which data
     * has been initialized in each frame's backing buffer.  Submit/rotate
     * before the overwrite, as the upstream renderer does.  Fast-fence mode
     * keeps this path deferred.
     */
