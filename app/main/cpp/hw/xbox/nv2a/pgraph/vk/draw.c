...
#if NV2A_PERF_LOG
#define VAF_STAT_INC(field) (g_vaf_stats.field++)
#else
#define VAF_STAT_INC(field) do { } while (0)
#endif
...
#else
    /* Profiling is compiled out in normal builds: do no per-frame logging or
     * counter maintenance on the render thread. */
#endif
}
...
void pgraph_vk_finalize_pipelines(PGRAPHState *pg)
{
    ...
    for (int i = 0; i < NUM_SUBMIT_FRAMES; i++) {
        ...
        for (int j = 0; j < r->deferred_image_view_count[i]; j++) {
            vkDestroyImageView(r->device, r->deferred_image_views[i][j], NULL);
        }
        r->deferred_image_view_count[i] = 0;
...
static void push_texture_descriptors(PGRAPHState *pg)
{
    ...
    r->push_desc_layout = r->pipeline_binding->layout;
}
...
void pgraph_vk_flush_all_frames(PGRAPHState *pg)
{
    ...
    for (int i = 0; i < NUM_SUBMIT_FRAMES; i++) {
        ...
        if (r->frame_submitted[i]) {
            ...
            r->deferred_framebuffer_count[i] = 0;
            for (int j = 0; j < r->deferred_image_view_count[i]; j++) {
                vkDestroyImageView(r->device,
                                   r->deferred_image_views[i][j], NULL);
            }
            r->deferred_image_view_count[i] = 0;
        }
    ...
    /*
     * Recycling the descriptor ring is only safe once nothing can still be
     * referencing those sets. The fence waits above cover work already
     * SUBMITTED - they say nothing about a command buffer that is still open
     * and recording, and this function is reached from fifteen call sites
     * across texture.c, surface.c and display.c, plenty of which run mid
     * command buffer.
     *
     * Resetting the index there meant the next update rewrote a set the open
     * command buffer had already bound, which invalidates that command buffer.
     * The Khronos validation layer named it exactly:
     *
     *   vkCmdBeginRenderPass(): ... VkCommandBuffer ... is now in an invalid
     *   state (instead of recording state) because ... VkDescriptorSet ...
     *   was destroyed or updated without UPDATE_AFTER_BIND
     *
     * Everything after that point is undefined, and on stock Adreno it ends as
     * a SIGSEGV inside the driver. UPDATE_AFTER_BIND is not an option here:
     * the spec forbids it for VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, which
     * is what these UBO bindings use. So the sets simply must not be recycled
     * while still bound - the index keeps advancing instead, and the ring is
     * recycled at the next flush that happens outside a command buffer.
     */
    if (!r->in_command_buffer && !r->is_render_thread_context) {
...
void pgraph_vk_finish(PGRAPHState *pg, FinishReason finish_reason)
{
    ...
    if (r->in_command_buffer) {
        ...
        if (!r->is_render_thread_context ||
            ...
            if (qatomic_read(&r->frame_submitted[next_frame])) {
                ...
                for (int i = 0; i < r->deferred_image_view_count[next_frame]; i++) {
                    vkDestroyImageView(r->device,
                                       r->deferred_image_views[next_frame][i],
                                       NULL);
                }
                r->deferred_image_view_count[next_frame] = 0;
        ...
        /* Push descriptors do not survive a command buffer, so the recorded
         * layout is meaningless once this one ends. */
        r->push_desc_layout = VK_NULL_HANDLE;
    ...
    /* The next command buffer has no outstanding reads from newly uploaded
     * vertex ranges. Per-frame initialization tracking is intentionally kept
     * in frame_staging[].uploaded_bitmap and is not cleared here. */
            ...
            if (tex_vram_clean) {
                ...
                if (sfp_ok && pg->vertex_attr_gen != r->pipeline_vertex_attr_gen) {
                    ...
                    if (desc_match) {
                        ...
                        VAF_STAT_INC(sfp_vaf_hit);
                        if (r->use_push_constants_for_uniform_attrs &&
                            r->shader_binding->state.vsh.uniform_attrs) {
                            VAF_STAT_INC(sfp_vaf_uniform_pushed);
                        } else {
                            VAF_STAT_INC(sfp_vaf_no_uniform);
                        }
    ...
    if (r->bindless_textures_supported &&
        ...
        if (bl_ok && pg->vertex_attr_gen != r->pipeline_vertex_attr_gen) {
            ...
            if (bl_desc_match) {
                ...
                r->pipeline_num_active_bind_descs = r->num_active_vertex_binding_descriptions;
                push_vertex_attr_values(pg);
                VAF_STAT_INC(bl_vaf_hit);
                if (r->use_push_constants_for_uniform_attrs &&
                    r->shader_binding->state.vsh.uniform_attrs) {
                    VAF_STAT_INC(bl_vaf_uniform_pushed);
                }
                OPT_STAT_INC(vtx_attr_fast);
            } else {
                VAF_STAT_INC(bl_vaf_miss);
                bl_ok = false;
            }
        }
        if (bl_ok) {
        ...
        if (pg->vertex_attr_gen != r->pipeline_vertex_attr_gen) {
            if (g_vaf_disabled ||
                ...
                memcmp(r->vertex_binding_descriptions,
                       ...
                       r->num_active_vertex_binding_descriptions *
                           sizeof(r->vertex_binding_descriptions[0]))) {
                VAF_STAT_INC(mfp_vaf_miss);
                goto mfp_miss;
            }
            r->pipeline_vertex_attr_gen = pg->vertex_attr_gen;
            r->pipeline_num_active_attr_descs = r->num_active_vertex_attribute_descriptions;
            r->pipeline_num_active_bind_descs = r->num_active_vertex_binding_descriptions;
            VAF_STAT_INC(mfp_vaf_hit);
            if (r->use_push_constants_for_uniform_attrs &&
                r->shader_binding->state.vsh.uniform_attrs) {
                VAF_STAT_INC(mfp_vaf_uniform_pushed);
            }
            OPT_STAT_INC(vtx_attr_fast);
        }
...
mfp_miss: (void)0;
    ...
    if (pg->vertex_attr_gen != r->pipeline_vertex_attr_gen) {
        VAF_STAT_INC(full_path_vag_changed);
    } else {
        VAF_STAT_INC(full_path_vag_same);
    }
...
static void begin_draw(PGRAPHState *pg)
{
    ...
    if (must_bind_pipeline) {
        ...
        /* Binding a pipeline whose layout is not compatible with the one the
         * push descriptors were recorded against invalidates set 0. Only
         * texture changes and command buffer boundaries marked it dirty, so a
         * pipeline switch left set 0 unbound for the next draw -
         * VUID-vkCmdDrawIndexed-None-08600, "uses set 0 but that set is not
         * bound". The proprietary Adreno driver keeps the stale binding alive
         * and renders correctly anyway; Turnip does not, which showed up as
         * striped surfaces and black polygon wedges in JSRF and Crash.
         * Only a layout change actually invalidates them, so tracking the
         * layout keeps this correct without paying a re-push on every bind. */
        if (r->push_desc_layout != r->pipeline_binding->layout) {
            r->push_tex_dirty = true;
        }
#if OPT_DYNAMIC_STATES
        r->dyn_state.valid = false;
    ...
    if (!pg->clearing) {
        ...
        /*
         * Uniform vertex attributes are push constants and may change between
         * draws even when the optimized pre-draw path reuses the pipeline and
         * descriptors.  Always refresh them: Xbox UI/font renderers commonly
         * encode each glyph's position and texture coordinates this way.
         */
        push_vertex_attr_values(pg);
    }
...
static bool vertex_ram_upload_hazard(PGRAPHVkState *r);
static void mark_vertex_ram_ranges_in_use(PGRAPHVkState *r);
...
static void flush_draw_queue_internal(NV2AState *d)
{
    ...
    if (is_indexed) {
        ...
        bool force_inline = vertex_ram_upload_hazard(r);
        if (force_inline) {
            r->num_vertex_ram_buffer_syncs = 0;
        } else {
        ...
        VertexBufferRemap remap =
            remap_unaligned_attributes(pg, max_element + 1, force_inline);
        ...
        VertexBufferRemap remap =
            remap_unaligned_attributes(pg, max_end, force_inline);
...
/*
 * Returns true when a guest write touches vertex RAM already referenced by
 * the command buffer currently being recorded.  Rewriting that mapped range
 * would retroactively change older draws, which is especially visible when
 * games reuse one small quad for every menu/font glyph.
 */
static bool vertex_ram_upload_hazard(PGRAPHVkState *r)
{
...
static void mark_vertex_ram_ranges_in_use(PGRAPHVkState *r)
{
    for (int i = 0; i < r->num_vertex_ram_buffer_syncs; i++) {
        ...
        size_t start_bit = addr / TARGET_PAGE_SIZE;
        size_t end_bit = end / TARGET_PAGE_SIZE;
        bitmap_set(r->uploaded_bitmap, start_bit, end_bit - start_bit);
    }
}
...
static bool try_snapshot_draw_arrays(NV2AState *d, ReorderWindowEntry *e)
{
    ...
    VertexBufferRemap remap =
        remap_unaligned_attributes(pg, max_element, force_inline);
...
static bool try_snapshot_inline_elements(NV2AState *d, ReorderWindowEntry *e)
{
    ...
    if (force_inline) {
        r->num_vertex_ram_buffer_syncs = 0;
    } else if (sync_range_covers(r, pg->vertex_attr_gen, min_element, max_element) &&
        !has_dirty_vertex_pages(r)) {
        OPT_STAT_INC(sync_range_skip);
        mark_vertex_ram_ranges_in_use(r);
        r->num_vertex_ram_buffer_syncs = 0;
    } else {
        sync_vertex_ram_buffer(pg);
                                                    ...
                                                    bool force_inline)
{
    ...
    for (int attr_id = 0; attr_id < NV2A_VERTEXSHADER_ATTRIBUTES; attr_id++) {
        ...
        if (!force_inline && offset_valid && stride_valid) {
            continue;
        }
                                                      ...
                                                      uint32_t num_vertices)
{
    ...
    // Copy vertex data (optimized: single memcpy when stride unchanged)
    for (int attr_id = 0; attr_id < NV2A_VERTEXSHADER_ATTRIBUTES; attr_id++) {
        ...
        if (old_stride == new_stride) {
            size_t total = (size_t)new_stride * num_vertices;
            memcpy(out_ptr, in_ptr, total);
        } else {
