...
#ifdef __ANDROID__
    /*
     * Debug builds package libVkLayer_khronos_validation.so and set
     * XEMU_VK_VALIDATION=1 when debug logging is on, which is the only way to
     * see descriptor-set lifetime and thread-ownership errors - the two things
     * the hand-written checks in shaders.c structurally cannot detect. The
     * release variant ships no layer, so this can never turn on there.
     */
    {
        const char *validation_env = getenv("XEMU_VK_VALIDATION");
        if (validation_env && validation_env[0] == '1') {
...
#ifdef __ANDROID__
        /*
         * Same story as push descriptors and EDS3 blend above: the stock
         * Qualcomm driver advertises descriptor indexing and then faults
         * inside vkUpdateDescriptorSets() during real draws (SIGSEGV at a
         * null+0x28 on Adreno 750, reached from pgraph_vk_update_descriptor_
         * sets -> begin_pre_draw). Updated drivers are fine, so only stock
         * Adreno is forced onto the plain descriptor path.
         */
        extern bool xemu_android_vulkan_custom_driver_zip_loaded(void);
        if (r->device_props.vendorID == 0x5143u &&
            !xemu_android_vulkan_custom_driver_zip_loaded()) {
            ...
            __android_log_print(ANDROID_LOG_INFO, "hakuX",
                                "Qualcomm stock driver: bindless textures disabled");
        } else
#endif
