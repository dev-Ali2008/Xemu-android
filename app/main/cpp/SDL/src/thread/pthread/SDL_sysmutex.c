...
void SDL_DestroyMutex(SDL_mutex *mutex)
{
    if (mutex) {
#ifndef __ANDROID__
        /* FIX: On Android, SDL_DestroyMutex is called during SDL_Quit()
         * while QEMU threads may still hold or lock this mutex, causing
         * bionic FORTIFY: pthread_mutex_lock on a destroyed mutex crashes.
         * We skip the destroy and let the OS reclaim it on _exit(0). */
