...
__attribute__((constructor)) static void resolve_linker_symbols() {
    ...
    // XaniteOG: these paths all returned silently, which made a failure here
    // indistinguishable from "no custom driver installed". Log which step fails
    // so Turnip loading problems are diagnosable from logcat.
#define LNB_FAIL(step) do { \
        __android_log_print(ANDROID_LOG_WARN, "linkernsbypass", \
                            "init failed at %s (api=%d)", (step), \
    ...
    if (!ldHandle)
        LNB_FAIL("loader_dlopen(ld-android.so)");
    ...
    if (!android_link_namespaces_all_libs)
        LNB_FAIL("dlsym(__loader_android_link_namespaces_all_libs)");
    ...
    if (!loader_android_create_namespace)
        LNB_FAIL("dlsym(__loader_android_create_namespace)");
    ...
    if (!android_get_exported_namespace)
        LNB_FAIL("dlsym(__loader_android_get_exported_namespace)");
#undef LNB_FAIL

    __android_log_print(ANDROID_LOG_INFO, "linkernsbypass",
                        "init OK (api=%d)", android_get_device_api_level());
