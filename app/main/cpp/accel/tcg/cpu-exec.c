...
#define TIER1_PROMOTION_BUDGET   16   /* Max promotions per budget window */
#define TIER1_BUDGET_INTERVAL_MS 10   /* Reset budget every N ms */

static int tier1_promotion_budget = TIER1_PROMOTION_BUDGET;

/*
 * Promotions allowed per budget window, tunable per title.
 *
 * The default of 16 per 100000-iteration window is a ~0.016% promotion rate.
 * A block that crosses the execution threshold while the budget is spent is
 * dropped and stays in tier 0 - then re-checked and re-dropped on every
 * subsequent execution, so genuinely hot code can stay unpromoted for an
 * entire session. Forza Motorsport showed promoted=28800 against
 * dropped=179843782 with one vCPU thread pinned near 100%.
 *
 * Raising this lets a title's hot working set promote quickly, after which
 * drops fall away on their own - promotion is once per block. The cost is
 * more JIT compilation up front, which is why it stays per-title rather than
 * becoming a new global default.
 */
static int g_tier1_budget = TIER1_PROMOTION_BUDGET;

void xemu_set_tier1_budget(int value)
{
    /* 0 disables promotion entirely, so no CF_TIER1 block is ever generated.
     * This used to clamp to 1, which left no way to take tier-1 out of the
     * picture when diagnosing a hang without rebuilding. */
    if (value < 0) value = 0;
    if (value > 4096) value = 4096;
    g_tier1_budget = value;
}

int xemu_get_tier1_budget(void)
{
    return g_tier1_budget;
}

/*
 * Master switch for tier-1 promotion.  Separate from tier1_budget because
 * PerGameSettingsManager rewrites runtime_override_tier1_budget on every launch
 * for titles with a profile (Forza forces 512), so the budget cannot be used to
 * take tier-1 out of the picture.  This key has no per-game override, so it
 * sticks.  0 = never promote, i.e. no CF_TIER1 block is ever generated.
 *
 * Defaults OFF.  Promotion hangs Forza at the profile screen: the vCPU thread
 * stops returning to the dispatcher entirely (the tier1 counters below freeze
 * mid-run while the display keeps refreshing) and burns ~95% of a core inside
 * a handful of generated instructions.  Reproduced with tier1_opt_mask=0, so
 * it is not the dead-flag passes - it is CF_TIER1 codegen, superblock
 * formation or the hot arena, none of which ever executed before the
 * 2026-07-29 CF_PCREL promotion-key fix made promotion work for the first time.
 * Turn back on only once that hang is understood.
 */
static int g_tier1_enable = 0;

void xemu_set_tier1_enable(int value)
{
    g_tier1_enable = value ? 1 : 0;
}

int xemu_get_tier1_enable(void)
{
    return g_tier1_enable;
}
...
/* Diagnostics: the request table filled to exactly TIER1_REQUEST_SLOTS and
 * never drained, so nothing was ever consumed. These say whether the consume
 * path is reached at all, and whether the (pc, cs_base, flags) key matches. */
static uint64_t g_tier1_consume_calls;
static uint64_t g_tier1_consume_hits;
...
int tier1_consume_request(vaddr pc, uint64_t cs_base, uint32_t flags,
                          uint32_t *cflags_out)
{
    g_tier1_consume_calls++;
#ifdef __ANDROID__
    /* One-shot: consume_hits stayed at 0 across 146k calls, so the key never
     * matches. Dump the query against a queued request to see which field
     * differs. */
    static int key_dumps;
    if (g_tier1_enable && key_dumps < 3) {
        for (int i = 0; i < TIER1_REQUEST_SLOTS; i++) {
            if (tier1_requests[i].valid) {
                key_dumps++;
                __android_log_print(ANDROID_LOG_INFO, "xanite-tier1",
                    "key query pc=0x%llx cs_base=0x%llx flags=0x%x | "
                    "slot%d pc=0x%llx cs_base=0x%llx flags=0x%x",
                    (unsigned long long)pc, (unsigned long long)cs_base, flags,
                    i, (unsigned long long)tier1_requests[i].pc,
                    (unsigned long long)tier1_requests[i].cs_base,
...
/*
 * @pc must be the virtual PC the block was looked up with, not tb->pc.
 * Under CF_PCREL tb->pc is not populated (it reads 0), which is why the rest
 * of this file compares it as `tb_cflags(tb) & CF_PCREL || tb->pc == s.pc`.
 * Recording tb->pc here stored 0, so tier1_consume_request() could never match
 * the real PC and no queued promotion was ever consumed: measured 149k consume
 * calls with 0 hits, and promoted frozen at TIER1_REQUEST_SLOTS.
 */
                                       ...
                                       vaddr pc)
{
    ...
    if (slot < 0) {
        /*
         * No free slot, so no tier-1 retranslation can be queued. Previously
         * the block was invalidated anyway: it came back as tier 0, re-crossed
         * the threshold, was "promoted" again, and was destroyed again - a
         * permanent invalidate/retranslate treadmill on the hottest code, on
         * the one thread everything else waits for. Measured on Forza:
         * promoted climbing forever at exactly the budget rate with
         * tb_flush_count stuck at 0. Leave the block alone instead and retry
                                       ...
                                       vaddr pc)
{
    ...
    if (tb->tier == 0 && tb->exec_count >= (uint32_t)g_tier1_threshold) {
        if (tier1_promotion_budget > 0) {
            /* Only spend budget on a promotion that was actually queued;
             * a failed queue used to consume budget and destroy the block. */
            if (tb_request_tier1_promotion(cpu, tb, pc)) {
 ...
 */
#define XBOX_SUPERBLOCK_ENABLED 1
...
static inline void tier1_maybe_reset_budget(void)
{
    if (++tier1_budget_counter >= TIER1_BUDGET_RESET_INTERVAL) {
        ...
        tier1_promotion_budget = g_tier1_budget;
        superblock_budget = SUPERBLOCK_BUDGET;
        /* Diagnostic scaffolding from the promotion investigation. Silent
         * unless someone turns tier-1 back on to debug the hang, so release
         * builds (which ship with promotion off) log nothing. */
        if (g_tier1_enable && ++tier1_log_counter >= TIER1_LOG_INTERVAL) {
            tier1_log_counter = 0;
...
#ifdef __ANDROID__
            /* qemu_printf() ends up on stdout, which is discarded on Android,
             * so these counters were invisible on the one platform where the
             * JIT promotion budget actually matters. Rate-limited already by
             * TIER1_LOG_INTERVAL. */
            /* tb_flush count included because promotions running permanently
             * at the budget rate means blocks keep losing tier-1 status; a
             * climbing flush count would explain that as code-cache thrash. */
            __android_log_print(ANDROID_LOG_INFO, "xanite-tier1",
                                "threshold=%d budget=%d promoted=%lu dropped=%lu "
                                "flushes=%u consume_calls=%lu consume_hits=%lu",
                                g_tier1_threshold, g_tier1_budget,
                                (unsigned long)g_tier1_promotions_total,
                                (unsigned long)g_tier1_promotions_dropped,
                                qatomic_read(&tb_ctx.tb_flush_count),
                                (unsigned long)g_tier1_consume_calls,
                                (unsigned long)g_tier1_consume_hits);
#endif
        }
    }
}
