...
void MainMenuGeneralView::Draw()
{
    ...
    Toggle("Native Floats (JIT)", &g_config.perf.fp_jit,
           "Use native float storage and inline FP ops (requires restart)");
#endif
...
void MainMenuDisplayView::Draw()
{
    ...
    float current = nv2a_get_surface_scale_factor();
    int rendering_scale = current < 1.0f ? 0 : (int)current;
    if (ChevronCombo("Internal resolution scale", &rendering_scale,
        ...
        nv2a_set_surface_scale_factor(rendering_scale == 0 ? 0.5f : (float)rendering_scale);
    }
