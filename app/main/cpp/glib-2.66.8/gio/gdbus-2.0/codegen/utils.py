...
def _version_key(version):
    """Return a loose, comparable version key without distutils.

    distutils was removed in Python 3.12.  GLib's D-Bus generator only needs
    numeric `Since` versions (plus the UNRELEASED sentinel handled below), so
    tokenising numbers and text preserves the ordering without adding a host
    Python dependency.
    """
    return tuple(
...
def version_cmp_key(key):
    ...
    # allow the version key to work and will always compare lower.
    if key[0] == 'UNRELEASED':
        v = '9999'
    elif key[0]:
        v = str(key[0])
