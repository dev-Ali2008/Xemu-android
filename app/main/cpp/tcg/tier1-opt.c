...
/*
 * Which tier-1 passes are allowed to run.  Both passes were unreachable until
 * the CF_PCREL promotion-key fix landed (2026-07-29), so neither has ever been
 * validated against a running title.  Both are off by default:
 *
 *   - cross-TB DFE is unsound as designed, see the comment on
 *     tier1_cross_tb_dead_flag_elimination().
 *   - intra-TB DFE is suspect on measurement.  With mask=0x1 on Forza, 71% of
 *     all process CPU collapsed into nine consecutive instructions of one
 *     generated block; with mask=0x0 that concentration disappears entirely and
 *     CPU spreads normally across DSP/APU emulation.  Not yet explained, and
 *     not yet distinguished from a legitimately tight optimised loop - it needs
 *     an A/B in a real race, not a menu.
 *
 * Runtime-settable so the passes can be bisected without a rebuild each time.
 */
int g_tier1_opt_mask = 0;

void xemu_set_tier1_opt_mask(int value)
{
    g_tier1_opt_mask = value & (TIER1_OPT_INTRA_DFE | TIER1_OPT_CROSS_TB_DFE);
}

int xemu_get_tier1_opt_mask(void)
{
    return g_tier1_opt_mask;
}
 ...
 *
 * UNSOUND AS WRITTEN - disabled by default via g_tier1_opt_mask.  The
 * successor lookup in translate-all.c is best-effort, but the result is used
 * as a correctness precondition rather than a hint:
 *
 *   1. The successor is a translation-time snapshot.  When it is invalidated
 *      the predecessor is NOT retranslated, and its flag stores are already
 *      gone.  tb_request_tier1_promotion() calls tb_phys_invalidate() on every
 *      promotion, so tier-1 itself churns the successors this pass relies on.
 *   2. goto_tb does not guarantee reaching that successor.  Until tb_add_jump()
 *      links it, and after any tb_reset_jump() unlink, control exits to the
 *      dispatcher and may resume anywhere - including a path that reads the
 *      flags this pass deleted.
 *   3. The lookup key is a guess: tb_hash_func(target, target, ...) reuses
 *      target as cs_base and the predecessor's flags, so under CF_PCREL it can
 *      match a different TB and apply the wrong cc_defines_first mask.
 *
 * Any of these leaves a CC global stale, so a conditional branch tests a
 * garbage flag.  Observed on Forza 2026-07-29 as a guest loop that never
 * exited: one vCPU pinned at 93% with 100% of JIT time inside nine
 * instructions.  Making this sound needs a reverse-dependency list so
 * invalidating a TB also invalidates predecessors compiled against it.
 */
