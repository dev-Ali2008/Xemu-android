...
void helper_fcomi_ST0_FT0(CPUX86State *env)
{
...
#ifdef XBOX
    /* Fable's scene-query list can become circular during zone transitions.
     * This is restricted to the three known Fable title IDs and to the exact
     * guest comparison site used by that list. Repeated visits set the
     * game's own skip flag on the cycling node. */
    if (g_game_quirks.fable_scene_graph_cycle_breaker &&
        ...
        } else if (num_seen < (int)ARRAY_SIZE(seen_esi)) {
