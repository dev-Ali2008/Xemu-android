...
constexpr const char* kPrefsName = "xanite_prefs";
...
static SetupFiles SyncSetupFiles() {
  ...
  std::string base = std::string(basePath) + "/Xanite";
...
extern "C" float xemu_android_get_fps(void);
...
{
    float current = xemu_android_get_fps();

    static int s_log_counter = 0;
    if ((s_log_counter++ % 20) == 0) {         
        __android_log_print(ANDROID_LOG_INFO, kLogTag,
                            "nativeGetFps: %.2f", current);
    }

    if (current < 0.0f) {
        return 0.0f;
    }
    return static_cast<jfloat>(current);
}
