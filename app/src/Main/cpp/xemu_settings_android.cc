...
void xemu_settings_save(void)
{
    ...
    if (!path || *path == '\0') {
        __android_log_print(ANDROID_LOG_ERROR, "xemu-android",
                            "xemu_settings_save: no config path set");
        return;
    }
    ...
    try {
        ...
        auto EnsureTable = [](toml::table &parent,
                               std::string_view key) -> toml::table * {
            if (auto *node = parent.get(key)) {
                if (auto *t = node->as_table()) return t;
            }
        ...
        toml::table *sys_files      = EnsureTable(*sys, "files");


        general->insert_or_assign("show_welcome",
                                  (bool)g_config.general.show_welcome);
        general->insert_or_assign("skip_boot_anim",
                                  (bool)g_config.general.skip_boot_anim);


        {
            const char *renderer_str = "opengl";
            if (g_config.display.renderer == CONFIG_DISPLAY_RENDERER_VULKAN)
                renderer_str = "vulkan";
            else if (g_config.display.renderer == CONFIG_DISPLAY_RENDERER_NULL)
                renderer_str = "null";
            display->insert_or_assign("renderer",
                                      std::string(renderer_str));
        }
        {
            const char *filtering_str =
                (g_config.display.filtering == CONFIG_DISPLAY_FILTERING_NEAREST)
                    ? "nearest"
                    : "linear";
            display->insert_or_assign("filtering",
                                      std::string(filtering_str));
        }
        {
            int scale = g_config.display.quality.surface_scale;
            if (scale < 1) scale = 1;
            if (scale > 3) scale = 3;
            display_quality->insert_or_assign("surface_scale",
                                              (int64_t)scale);
        }
        display_window->insert_or_assign("vsync",
                                         (bool)g_config.display.window.vsync);


        audio->insert_or_assign("use_dsp", (bool)g_config.audio.use_dsp);
        audio->insert_or_assign("hrtf",    (bool)g_config.audio.hrtf);
        {
            double vol = g_config.audio.volume_limit;
            if (vol < 0.0) vol = 0.0;
            if (vol > 1.0) vol = 1.0;
            audio->insert_or_assign("volume_limit", vol);
        }
        audio_vp->insert_or_assign("num_workers",
                                    (int64_t)g_config.audio.vp.num_workers);


        perf->insert_or_assign("hard_fpu",
                                (bool)g_config.perf.hard_fpu);
        perf->insert_or_assign("cache_shaders",
                                (bool)g_config.perf.cache_shaders);


        sys->insert_or_assign("mem_limit",
            std::string(g_config.sys.mem_limit == CONFIG_SYS_MEM_LIMIT_128
                            ? "128"
                            : "64"));
        ...
        if (g_config.sys.files.bootrom_path &&
            *g_config.sys.files.bootrom_path) {
            sys_files->insert_or_assign(
                "bootrom_path",
                std::string(g_config.sys.files.bootrom_path));
        }
        if (g_config.sys.files.flashrom_path &&
            *g_config.sys.files.flashrom_path) {
            sys_files->insert_or_assign(
                "flashrom_path",
                std::string(g_config.sys.files.flashrom_path));
        }
        if (g_config.sys.files.hdd_path && *g_config.sys.files.hdd_path) {
            sys_files->insert_or_assign(
                "hdd_path",
                std::string(g_config.sys.files.hdd_path));
        }
        if (g_config.sys.files.dvd_path && *g_config.sys.files.dvd_path) {
            sys_files->insert_or_assign(
                "dvd_path",
                std::string(g_config.sys.files.dvd_path));
        }
        if (g_config.sys.files.eeprom_path &&
            *g_config.sys.files.eeprom_path) {
            sys_files->insert_or_assign(
                "eeprom_path",
                std::string(g_config.sys.files.eeprom_path));
        }
        ...
        if (!out.is_open()) {
            __android_log_print(ANDROID_LOG_ERROR, "xemu-android",
                                "xemu_settings_save: cannot open %s for writing",
                                path);
            return;
        }
        out << tbl;
        ...
        __android_log_print(ANDROID_LOG_INFO, "xemu-android",
                            "xemu_settings_save: saved to %s", path);
    } catch (const std::exception &e) {
        __android_log_print(ANDROID_LOG_ERROR, "xemu-android",
                            "xemu_settings_save: exception: %s", e.what());
    }
}
